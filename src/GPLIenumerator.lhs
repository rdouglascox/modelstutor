> module GPLIenumerator (enumerator, allPairs, allTriples) where
...
> cool :: [Char] -> [Int] -> [(Model,Bool)]
> cool x y = zip (getmodels x y) (map (newval x) (getmodels x y))

> enumerator x y = do 
>                  mapM_ (putStrLn . (("Formula: " ++ x ++ "\n") ++) . prettyprint) (cool x y)
>                  putStrLn ("Total number of possible models: " ++ (show (length (cool x y))))

> prettyprint :: (Model,Bool) -> [Char]
> prettyprint (mod,val) = "Model:\n Domain: " ++ show (domain mod) ++ "\n Referents: " ++ show (referents mod) ++ "\n Extensions:\n    One-place: " ++ show (extensions' mod) ++ "\n    Two-place: " ++ show (extensions'' mod) ++ "\n    Three-place: " ++ show (extensions''' mod) ++ "\nValue: " ++ show val ++ "\n"
